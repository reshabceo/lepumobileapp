    ...
    public void getEcgRecord(PluginCall call) {
        ...
        try {
            ...
            if (bleHelper != null) {
                ...
                try {
                    java.lang.reflect.Method getConnectedDevices = bleHelper.getClass()
        ...
        try {
            ...
            if (!isBp2w) {
                java.lang.reflect.Method bp2ReadFile = bleHelper.getClass().getMethod("bp2ReadFile", int.class,
                        String.class);
                bp2ReadFile.invoke(bleHelper, currentModel, recordId);
            } else {
                java.lang.reflect.Method bp2wReadFile = bleHelper.getClass().getMethod("bp2wReadFile", int.class,
                        String.class);
                bp2wReadFile.invoke(bleHelper, currentModel, recordId);
            }

            // Set up observers with proper cleanup to prevent memory leaks and duplicate
            // resolves
            ...
            final java.util.concurrent.atomic.AtomicBoolean handled = new java.util.concurrent.atomic.AtomicBoolean(
                    false);
            ...
            if (m.getParameterCount() == 0) {
                try {
                    ...
                    if (rt != null && (rt.getSimpleName().contains("EcgFile") || rt.getName().endsWith(".EcgFile"))) {
                        ...
                        if (r != null) {
                            Log.d("Bp2Plugin",
                                    "unwrapEcgFile: via method " + m.getName() + "() -> " + r.getClass().getName());
                            return r;
                        }
                    }
                } catch (Throwable ignore) {
            ...
            for (String alt : new String[] { "getShortData", "getWaveData" }) {
                try {
                    ...
                    if (r instanceof short[]) {
                        short[] shorts = (short[]) r;
                        Log.d("Bp2Plugin",
                                "SUCCESS: Found short[] data via " + alt + ": " + shorts.length + " samples");
                        return shorts;
                    }
                    if (r instanceof byte[]) {
                        byte[] b = (byte[]) r;
                        ...
                        for (int i = 0; i < s.length; i++)
                            s[i] = bb.getShort();
                        Log.d("Bp2Plugin",
                                "SUCCESS: Converted byte[] to short[] via " + alt + ": " + s.length + " samples");
                        return s;
                    }
                } catch (Throwable ignore) {
                }
            ...
            try {
                ...
                for (var m : ecgCls.getMethods()) {
                    if (!java.lang.reflect.Modifier.isStatic(m.getModifiers()))
                        continue;
                    if (!ecgCls.isAssignableFrom(m.getReturnType()))
                        continue;
                ...
                try {
                    ...
                    for (var m : ecgCls.getMethods()) {
                        var name = m.getName();
                        if (!(name.equals("parse") || name.equals("decode") || name.equals("fromBytes")
                                || name.equals("load")))
                            continue;
                        var params = m.getParameterTypes();
                        if (params.length >= 1 && params[0] == byte[].class) {
