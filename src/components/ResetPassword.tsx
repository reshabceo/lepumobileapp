...
export const ResetPassword: React.FC<ResetPasswordProps> = ({ onSuccess, onBack, embedded = false }) => {
  ...
  const formContent = (
    <div className={embedded ? "space-y-4" : "space-y-6"}>
      {/* Success Icon */}
      <div className={`flex justify-center ${embedded ? "mb-2" : "mb-6"}`}>
        <div className={`${embedded ? "w-14 h-14" : "w-20 h-20"} bg-green-600/20 rounded-2xl flex items-center justify-center backdrop-blur-sm border border-green-500/30`}>
          <CheckCircle2 className={`${embedded ? "w-7 h-7" : "w-10 h-10"} text-green-400`} />
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div className="relative group">
          <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
            <Lock className="text-gray-400 group-focus-within:text-blue-400 transition-colors duration-300" size={18} />
          </div>
          <input
            ...
            className={`w-full pl-11 pr-11 py-3.5 bg-black/30 backdrop-blur-sm text-white border ${
              errors.password ? 'border-red-500/50' : 'border-white/20'
            } rounded-2xl focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500/50 placeholder-gray-400 transition-all duration-300 text-sm`}
            disabled={loading}
          />
          <button
            type="button"
      ...
      <div className="mt-5 p-3.5 bg-black/20 rounded-xl border border-white/10">
        <p className="text-[10px] text-gray-400 mb-1.5 font-bold uppercase tracking-wide">Requirements:</p>
        <ul className="text-[11px] text-gray-500 space-y-1">
          <li className="flex items-center gap-2">
            <span className={formData.password.length >= 8 ? 'text-green-400 font-bold' : ''}>
              {formData.password.length >= 8 ? '✓' : '○'}
            </span>
            At least 8 characters
          </li>
          <li className="flex items-center gap-2">
            <span className={/(?=.*[a-z])/.test(formData.password) ? 'text-green-400 font-bold' : ''}>
          ...
          <li className="flex items-center gap-2">
            <span className={/(?=.*\d)/.test(formData.password) ? 'text-green-400 font-bold' : ''}>
