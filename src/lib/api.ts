...
const getApiBaseUrl = () => {
    ...
    if (typeof window !== 'undefined') {
        ...
        console.log('🔍 API Debug - Is Mobile:', isMobile);
        
        if (isMobile) {
            // For mobile devices (Android emulator/device), use the computer's IP address
            const mobileUrl = 'http://192.168.1.11:3000/api';
            console.log('🔍 API Debug - Using Mobile URL:', mobileUrl);
            return mobileUrl;
        }
    }
    // For web browser, use localhost
    const webUrl = 'http://localhost:3000/api';
    console.log('🔍 API Debug - Using Web URL:', webUrl);
    return webUrl;
};
...
class ApiService {
    ...
    async login(credentials: LoginRequest): Promise<LoginResponse> {
        ...
        console.log('🔍 Login Debug - API URL:', this.baseUrl);
